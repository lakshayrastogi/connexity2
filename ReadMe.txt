...
Instructions for running/setting up the website:
	...
	Steps to run if given root folder:
	...
	3)  Type and run command:  java -jar target/connexity2-0.0.1-SNAPSHOT.jar
		If error occurs where address in use is sited follow these instructions and then redo #2:
	...
	5)  In MAMP check to ensure port number is correct by going to preferences->ports and checking to see if the
	...
	4)  In MAMP check to ensure port number is correct by going to preferences->ports and checking to see if the
	MySQL Port number is 8889.
